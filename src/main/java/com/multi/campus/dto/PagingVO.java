...
public class PagingVO {
	...
	private int lastPageRecord = 0; //마지막 페이지에 남아 있는 레코드 수
		
	private int onePageNumCount=5; //한 페이지 당 페이지 개수
	...
	public void setTotalRecord(int totalRecord) {
		...
		if(nowPage==totalPage) {
			if(totalRecord%onePageRecord==0){//나머지가 0일 경우 lastPageRecord <- onePageRecord
				lastPageRecord = onePageRecord;
			}else{//나머지가 있는 경우 남아 있는 레코드 수를 lastPageRecord에 대입
